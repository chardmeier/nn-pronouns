function [best_weights, trainerr, valerr, best_valerr] = nnopt_deplm1(id, net, input, params)
...
%   Detailed explanation goes here
    ...
    if isfield(net, 'initweights')
        ...
        W = init_weights_deplm1(net, @(x,y) stddev * randn(x,y));
    end
    
    if isfield(params, 'initgain')
        gain = params.initgain;
    else
        gain = init_weights_deplm1(net, @ones);
    end
        
    weight_change = init_weights_deplm1(net, @zeros);
    prev_grad = init_weights_deplm1(net, @zeros);
    rms = init_weights_deplm1(net, @ones);
    best_valerr = inf;
    alphachange_steps = 0;
    tiny = 1e-30;
    ...
    for i = 1:nsteps
        ...
        for j = 1:nbatch
            ...
            batchinput = create_batch_deplm1(input, thisperm);
            Wstruct = weightstruct_deplm1(net, W);
            [output,hidden] = fprop_deplm1(net, batchinput, Wstruct, false);
            gradstruct = bprop_deplm1(net, batchinput, hidden, output, Wstruct);
            grad = weightvector_deplm1(gradstruct);
            W = weightvector_deplm1(Wstruct);
            ...
            if isfield(net, 'weightconstraint')
                weights = enforce_constraints_deplm1(net, weights);
            end
...
%         end
        ...
        Wstruct = weightstruct_deplm1(net, W);
        valout = fprop_deplm1(net, input.val, Wstruct, true);
...
%         end
        ...
        %if mod(i, 5) == 0
            ...
            save('-v7.3', sprintf('deplm1-%s.mat', id));
        %end
    end
